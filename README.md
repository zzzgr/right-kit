...
| 界面 | 出现时机 |
|------|---------|
| 设置向导 | 首次启动；之后从菜单栏警告或「通用」里重开 |
| 主窗口 | 菜单栏「设置…」（⌘,）、「我的动作…」、「动作市场…」 |
| 菜单栏下拉 | 设置… / 我的动作… / 动作市场… / 退出；扩展关闭时显示 ⚠ |

主窗口采用与「系统设置」一致的侧边栏布局，最小尺寸 1040 × 700，侧边栏顶部常驻扩展健康状态：

- **通用**：Finder 扩展状态与系统设置入口、重新打开向导、登录时启动、外观（跟随系统 / 明亮 / 黑暗，只影响自己的窗口）。
- **右键菜单**：每一行就是一个菜单项，右边紧跟着它会打开哪个 App；右侧「菜单预览」实时显示 Finder 里「右键助手」子菜单的样子，包括自定义动作及其分组。
- **我的动作**：左侧列表支持搜索、拖动排序（或 ⌘⌥↑ / ⌘⌥↓）和行内启用开关；右侧是所选动作的「动作设置 / 脚本 / 手动测试」。运行记录位于手动测试中，运行中的任务显示在侧边栏底部。
- **动作市场**：卡片网格，支持搜索、分类多选、运行环境筛选、仅看可更新与分页（每页 20 / 50 / 100）；分类对应动作分组，可同时查看图片工具、文件工具等多个类别。点击卡片在弹窗中查看说明、源代码与历史版本，再导入为草稿。右上角「市场设置」配置一个市场地址，支持 HTTP 和 HTTPS，包括局域网地址。
- **关于**：版本、下载、GitHub、指南与支持的 App。

动作图标统一为单色线条（跟随文字颜色），与网页市场一致。图标库提供文件、图片、办公、开发、网络和工具六类共 64 个内置图标，支持搜索；市场导入的自定义图标继续显示，也可从图标库替换。动作详情及升级日志可直接阅读 Markdown 标题、列表、代码块和参数表格。脚本编辑器、市场源码页和说明中的代码块提供适配浅深色模式的语法高亮，复制和保存保留原始代码。
...
```
Apps/RightKit/                  菜单栏 App：设置向导 / 主窗口（通用、右键菜单、我的动作、市场）/ 执行 handoff
