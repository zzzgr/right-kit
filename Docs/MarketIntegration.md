...
客户端的动作说明和升级日志支持 Markdown 标题、粗体、列表、引用、代码块、任务列表、链接和参数表格。源码页与「我的动作」脚本编辑器支持 Python / Shell 语法高亮，说明中的代码块另外支持 JSON，并适配浅色与深色外观。长代码可横向滚动、搜索和复制；高亮不修改脚本内容。图片保留替代文字，预览说明时不会自动加载外部图片。
...
动作市场默认每页 20 条，可切换为 50 / 100 条。点击「全部分类」可以勾选多个动作分组，例如图片工具和文件工具，列表显示所选分组的合集；再次点击「全部分类」清除分类限制。分类可与搜索、语言和「仅看可更新」组合使用，切换条件时回到第一页。筛选覆盖整个市场，再返回当前页；分类选项不会因搜索或空结果而消失。「仅看可更新」按已安装动作的 ID 查询，避免漏掉其他页面的更新。页面缓存按查询地址分别保存，离线时只展示对应查询的缓存。旧静态市场没有分页元信息时使用完整读取兼容。
...
分页查询使用 `page`、`pageSize`、`q`、`language`，默认 20 条，单页最多 100 条；更新查询额外传入 `ids`。分类通过重复的 `group` 参数传递，例如 `group=图片工具&group=文件工具`，按任一分组匹配；`group=` 表示未分组，省略则不限制。最多选择 100 个分组，每个名称最多 60 个 UTF-16 单元，分组名称中的逗号不作为分隔符。目录项目包含可选的 `group`，顶层 `groups` 提供整个已发布市场的分类集合；旧目录缺少这些字段时仍可读取。

目录返回可选的 `pagination: { page, pageSize, total, totalPages }`，并保留携带全部筛选条件的 `next` 链接。服务端对已发布内容完成搜索和筛选后分页，不会把草稿发给客户端。后台和网页使用相同的查询规则。
