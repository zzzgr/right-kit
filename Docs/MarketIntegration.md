...
RightKit 1.2.1 起支持动作市场接入。市场负责动作编辑、版本发布与升级日志；客户端负责检查内容、配置本机环境和执行保存后的动作。网页与客户端无需共享数据库或管理员登录。
...
网页的「添加到 RightKit」可以配置当前市场，也可以在客户端主窗口侧边栏切换到「动作市场」，打开右上角的「市场设置」，填写一个市场首页或目录地址。市场以卡片网格展示，点击卡片后在弹窗中查看详情。HTTP 和 HTTPS 均可使用，例如 `http://192.168.1.2:3000`。尚未配置市场时，首次导入经过验证的市场动作会保存对应市场地址。
