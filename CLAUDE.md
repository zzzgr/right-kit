...
| App Group | `group.com.rightkit.app` (single constant: `Preferences.appGroupID`, must match both entitlements) |
...
./Scripts/release.sh                          # Developer ID + notarize + staple → dist/*.dmg
```
...
the exception — it writes the pasteboard in the extension and only falls back to the
handoff on failure.
...
- **Terminals are opened via LaunchServices, never AppleScript.** Terminal.app
  ...
  and no Automation permission**. Adding a terminal means checking its
  `CFBundleDocumentTypes` for a folder type first.
...
- **Action resolution by `tag`.** Finder strips `representedObject`; the index maps into
  the `menuActions` snapshot taken while building the menu.
...
- **Verification is real, not assumed.** The extension writes `loadedAt` / `lastMenuAt`
  ...
  were not, nothing would ever arrive.
...
- **No Accessibility, no Automation, no notification permission at launch.** Notification
  authorization is requested on the *first failure* only.
...
- `Strings` — all copy, Chinese-primary with English fallback. No `.strings` files.
...
- Signing team lives in `project.yml` (`DEVELOPMENT_TEAM`). Override per build with
  `DEVELOPMENT_TEAM=<team> ./Scripts/install-local.sh` — there are deliberately no
